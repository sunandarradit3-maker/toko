...
const envSchema = z.object({
  ...
  PAYMENT_BCA: z.string().min(1),
  PAYMENT_MANDIRI: z.string().min(1),
  PAYMENT_BRI: z.string().min(1),
...
export function getTelegramAdminIds(): Set<string> {
  return new Set(getEnv().TELEGRAM_ADMIN_IDS.split(',').map((x) => x.trim()).filter(Boolean));
}
